...
function Assert-ResourcePowerState
{
    ...
    $function_Messages.Standard = $function_messages.Standard + "`n`tCurrent power state is [$currenPowerState]"
...
$calling_AssertResourcePowerState = {
    ...
    # $this_Run.PowerState_Messages = Assert-ResourcePowerState -Resource $resource -DesiredState $resource.DesiredState -Simulate $Simulate -StartTime $StartTime
...
try
{
    ...
    foreach($resource in $resourceList | Group-Object ScheduleMatched) {                # Groups all resources by the value of 'ScheduleMatched' (which is a boolean).  It then iterates through each group that was made
        ...
        {  
            ...
            Write-Output "Just before creating the job parameters"
